...
fn pending_items_drain_through_limited_slots() {
    ...
    assert!(all_done, "all 4 items should complete, proving pending items drained through slots");
}
...
fn failed_job_frees_slot_for_next_pending_item() {
    ...
    assert!(second_completed, "second item should complete, proving the failed job freed the slot");

    // Verify: 1 dead (the failed one), 1 completed
    let items = temp.oj().args(&["queue", "show", "tasks"]).passes().stdout();
    assert!(items.contains("completed"), "second item should be completed, got:\n{}", items);
    assert!(items.contains("dead"), "the failed item should be dead, got:\n{}", items);
}
...
fn worker_list_reflects_active_job_count() {
    ...
    // Each item needs unique data to avoid deduplication.
    temp.oj().args(&["queue", "push", "tasks", r#"{"cmd": "sleep 30 && echo item-0"}"#]).passes();
    temp.oj().args(&["queue", "push", "tasks", r#"{"cmd": "sleep 30 && echo item-1"}"#]).passes();
...
fn independent_workers_have_separate_concurrency_pools() {
    ...
    temp.oj().args(&["daemon", "start"]).passes();
    temp.oj().args(&["worker", "start", "alpha_runner"]).passes();
    ...
    temp.oj().args(&["queue", "push", "alpha", r#"{"cmd": "sleep 30 && echo alpha"}"#]).passes();
    temp.oj().args(&["queue", "push", "beta", r#"{"cmd": "sleep 30 && echo beta"}"#]).passes();
    ...
    let job_output = temp.oj().args(&["job", "list"]).passes().stdout();
    assert!(job_output.contains("alpha_job"), "should see alpha_job job:\n{}", job_output);
    assert!(job_output.contains("beta_job"), "should see beta_job job:\n{}", job_output);
}
