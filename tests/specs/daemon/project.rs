...
/// Runbook with a command that runs a job echoing OJ_PROJECT.
...
impl ProjectPair {
    fn new(name_a: &str, name_b: &str) -> Self {
        ...
        Self { project_a, project_b, shared_state }
    }
    ...
    fn oj_a(&self) -> CliBuilder {
        cli()
            ...
            .env("CLAUDE_CONFIG_DIR", PathBuf::from(self.state_path()).join("claude"))
...
fn jobs_with_same_name_in_different_namespaces_dont_interfere() {
    ...
    // Start workers in both projects - each is scoped to its project
    ...
    pair.oj_a().args(&["queue", "push", "tasks", r#"{"msg": "from-alpha"}"#]).passes();
    pair.oj_b().args(&["queue", "push", "tasks", r#"{"msg": "from-beta"}"#]).passes();

    // Wait for jobs to complete in both namespaces (must use --project to scope
    // by project; without it, `oj job list` returns ALL jobs across namespaces)
    let both_done = wait_for(SPEC_WAIT_MAX_MS * 2, || {
        let out_a = pair.oj_a().args(&["--project", "alpha", "job", "list"]).passes().stdout();
        let out_b = pair.oj_b().args(&["--project", "beta", "job", "list"]).passes().stdout();
    ...
    // Verify each project only sees its own job (project scoping via --project)
    let jobs_a = pair.oj_a().args(&["--project", "alpha", "job", "list"]).passes().stdout();
    let jobs_b = pair.oj_b().args(&["--project", "beta", "job", "list"]).passes().stdout();
...
"#;
    ...
    pair.oj_a().args(&["queue", "push", "tasks", r#"{"msg": "item-one"}"#]).passes();
    pair.oj_a().args(&["queue", "push", "tasks", r#"{"msg": "item-two"}"#]).passes();
    pair.oj_b().args(&["queue", "push", "tasks", r#"{"msg": "item-beta"}"#]).passes();

    // Wait for all items to be visible (events may still be processing under load)
    let items_visible = wait_for(SPEC_WAIT_MAX_MS, || {
        let queue_a = pair.oj_a().args(&["queue", "show", "tasks"]).passes().stdout();
        let queue_b = pair.oj_b().args(&["queue", "show", "tasks"]).passes().stdout();
...
#[test]
fn oj_project_propagates_to_shell_steps() {
    let pair = ProjectPair::new("project-test", "other-ns");

    pair.file_a(".oj/runbooks/check.toml", NAMESPACE_ECHO_RUNBOOK);

    pair.oj_a().args(&["daemon", "start"]).passes();

    // Run a command that triggers a job echoing $OJ_PROJECT
    pair.oj_a().args(&["run", "check", "project-test"]).passes();
    ...
    // Check job logs for the project echoed by the shell step
    let job_logs = pair.oj_a().args(&["logs", job_id]).passes().stdout();

    assert!(
        job_logs.contains("project=project-test"),
        "shell step should see OJ_PROJECT=project-test, got:\n{}",
        job_logs
    );
}
...
fn workers_in_different_namespaces_are_independent() {
    ...
    // Start workers in both namespaces (same worker name, different project)
    pair.oj_a().args(&["worker", "start", "runner"]).passes();
    pair.oj_b().args(&["worker", "start", "runner"]).passes();

    // Push items to each queue
    pair.oj_a().args(&["queue", "push", "tasks", r#"{"msg": "alpha-work"}"#]).passes();
    pair.oj_b().args(&["queue", "push", "tasks", r#"{"msg": "beta-work"}"#]).passes();
    ...
    assert!(both_done, "jobs should complete via project-scoped workers");

    // Verify worker list shows both workers with their respective namespaces
    // The daemon shows all workers (not filtered by project) but with PROJECT column
...
fn config_name_takes_precedence_over_directory_name() {
    ...
    temp.git_init();
    temp.file(".oj/config.toml", "[project]\nname = \"custom-project\"\n");
    temp.file(".oj/runbooks/check.toml", NAMESPACE_ECHO_RUNBOOK);

    temp.oj().args(&["daemon", "start"]).passes();

    // Run command that triggers a job echoing project
    temp.oj().args(&["run", "check", "custom-project"]).passes();
    ...
    // Verify the project is from config, not from the temp directory name
    // Check via job logs which shows the echo output
    let job_logs = temp.oj().args(&["logs", job_id]).passes().stdout();

    assert!(
        job_logs.contains("project=custom-project"),
        "project should be 'custom-project' from config.toml, not directory basename, got:\n{}",
        job_logs
    );
}
