...
fn cron_started_creates_record() {
    ...
    assert_eq!(record.project, "myns");
    assert_eq!(record.status, "running");
    assert_eq!(record.interval, "30m");
    assert_eq!(record.target, oj_core::RunTarget::job("cleanup"));
}
...
fn cron_stopped_updates_status() {
    ...
    state.apply_event(&Event::CronStopped { cron: "janitor".to_string(), project: String::new() });
...
fn cron_deleted_empty_namespace() {
    ...
    state.apply_event(&Event::CronDeleted { cron: "janitor".to_string(), project: String::new() });
    assert!(!state.crons.contains_key("janitor"));
}
