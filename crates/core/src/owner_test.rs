...
fn serializes_as_string() {
    let job = OwnerId::Job(JobId::new("abc123"));
    assert_eq!(serde_json::to_string(&job).unwrap(), r#""job:abc123""#);

    let crew = OwnerId::Crew(CrewId::new("xyz789"));
    assert_eq!(serde_json::to_string(&crew).unwrap(), r#""crew:xyz789""#);
}
...
fn deserializes_from_string() {
    ...
    let owner: OwnerId = serde_json::from_str(r#""crew:run456""#).unwrap();
    assert!(matches!(owner, OwnerId::Crew(_)));
}

#[test]
fn roundtrip() {
    let original = OwnerId::Job(JobId::new("test"));
