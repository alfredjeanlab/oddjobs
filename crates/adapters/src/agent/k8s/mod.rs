...
mod adapter {
    ...
    use crate::agent::docker::http;
    use crate::agent::log_entry::AgentLogMessage;
    use crate::agent::remote::RemoteCoopClient;
    use crate::agent::{
    ...
    impl KubernetesAdapter {
        ...
        async fn k8s_spawn(
            ...
            let auth_token = crate::agent::generate_auth_token();
            ...
            let git_clone_cmd = if config.workspace_path.join(".git").exists()
                || !config.workspace_path.exists()
            {
                crate::agent::detect_git_remote(&config.project_path).await.map(|repo_url| {
                    let branch = crate::agent::detect_git_branch_blocking(&config.workspace_path);
            ...
            // Build agent command
            let command = crate::agent::augment_command_for_skip_permissions(&config.command);
            ...
            crate::agent::poll_until_ready(&addr, &auth_token, poll_ms, max_attempts, "k8s")
                .await?;
