    ...
    impl DockerAdapter {
        ...
        async fn docker_spawn(
            ...
            if config.workspace_path.join(".git").exists() || !config.workspace_path.exists() {
                ...
                if let Some(repo_url) = crate::agent::detect_git_remote(&config.project_path).await
                {
                    let branch = crate::agent::detect_git_branch(&config.workspace_path).await;
            ...
            let cred_env;
            if let Some(cred) = crate::credential::resolve() {
            ...
            crate::agent::poll_until_ready(&addr, &auth_token, poll_ms, max_attempts, "docker")
                .await?;
