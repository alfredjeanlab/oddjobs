...
fn run_short_help_shows_custom_output() {
    let err = cli_command().try_get_matches_from(["oj", "run", "-h"]).unwrap_err();
...
fn run_long_help_shows_custom_output() {
    let err = cli_command().try_get_matches_from(["oj", "run", "--help"]).unwrap_err();
    assert_eq!(err.kind(), ErrorKind::DisplayHelp);
    let help = err.to_string();
    assert!(
        help.contains("Usage: oj run <COMMAND>"),
...
fn help_subcommand_run_shows_custom_output() {
    let err = cli_command().try_get_matches_from(["oj", "help", "run"]).unwrap_err();
    assert_eq!(err.kind(), ErrorKind::DisplayHelp);
    let help = err.to_string();
    assert!(
        help.contains("Usage: oj run <COMMAND>"),
...
fn run_help_and_help_run_are_identical() {
    let run_h = cli_command().try_get_matches_from(["oj", "run", "-h"]).unwrap_err().to_string();
    let help_run =
        cli_command().try_get_matches_from(["oj", "help", "run"]).unwrap_err().to_string();
...
fn directory_flag_short_c() {
    let matches = cli_command().try_get_matches_from(["oj", "-C", "/tmp", "status"]).unwrap();
...
fn directory_flag_before_subcommand() {
    let matches =
        cli_command().try_get_matches_from(["oj", "-C", "/tmp", "queue", "list"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
    assert_eq!(cli.directory.unwrap(), PathBuf::from("/tmp"));
}
...
fn directory_flag_absent() {
    let matches = cli_command().try_get_matches_from(["oj", "status"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
...
fn project_flag_global() {
    let matches =
        cli_command().try_get_matches_from(["oj", "--project", "myproj", "queue", "list"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
...
#[test]
fn namespace_resolution_falls_back_to_project_path() {
    // With no --project flag and no OJ_PROJECT env, should resolve from project path
...
fn project_filter_is_some_when_flag_passed() {
    let matches =
        cli_command().try_get_matches_from(["oj", "--project", "wok", "job", "list"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
...
fn project_filter_is_none_when_flag_omitted() {
    let matches = cli_command().try_get_matches_from(["oj", "job", "list"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
    // Even if OJ_PROJECT is set, cli.project is None — filtering is skipped
    assert_eq!(cli.project, None);
}

// -- Subcommand --project is removed  ----------------------
...
fn subcommand_project_flag_no_longer_exists() {
    ...
    assert!(result.is_ok(), "global --project should still parse on subcommands");
}
...
fn missing_subcommand_produces_help_error() {
    let err = cli_command().try_get_matches_from(["oj", "project"]).unwrap_err();
    assert_eq!(
        err.kind(),
...
fn missing_subcommand_help_contains_commands() {
    let err = cli_command().try_get_matches_from(["oj", "job"]).unwrap_err();
    assert_eq!(err.kind(), ErrorKind::DisplayHelpOnMissingArgumentOrSubcommand);
    let help = err.to_string();
    assert!(help.contains("Job management"), "should show job description, got:\n{help}");
}
...
fn subcommand_help_includes_global_flags() {
    ...
    assert!(help.contains("-C"), "subcommand help should include -C global flag, got:\n{help}");
    assert!(
...
fn subcommand_help_shows_full_usage_prefix() {
    ...
    let project = super::find_subcommand(cmd, &["project"]);
    let help = crate::help::format_help(project);
    assert!(help.contains("oj project"), "usage line should include 'oj project', got:\n{help}");
}
...
fn cancel_accepts_multiple_ids() {
    let matches = cli_command().try_get_matches_from(["oj", "cancel", "abc", "def"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
...
fn resume_accepts_id_and_message() {
    let matches =
        cli_command().try_get_matches_from(["oj", "resume", "abc", "-m", "try again"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
...
fn resume_accepts_var_flags() {
    let matches =
        cli_command().try_get_matches_from(["oj", "resume", "abc", "--var", "key=val"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
...
fn resume_accepts_all_flag() {
    let matches = cli_command().try_get_matches_from(["oj", "resume", "--all"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
...
fn resume_all_with_kill() {
    let matches = cli_command().try_get_matches_from(["oj", "resume", "--all", "--kill"]).unwrap();
    let cli = Cli::from_arg_matches(&matches).unwrap();
    assert!(matches!(cli.command, Some(Commands::Resume { id, all, kill, .. })
