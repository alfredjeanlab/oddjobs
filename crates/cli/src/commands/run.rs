...
pub struct RunArgs {
    ...
    /// Attach to the agent's session after starting
    #[arg(long = "attach", conflicts_with = "no_attach")]
    pub attach: bool,

    /// Do not attach to the agent's session
...
pub fn prevalidate_command(
    ...
    let runbook = crate::load_runbook(project_path, command, runbook_file)?;
...
pub fn available_commands_help(project_path: &Path) -> String {
    let runbook_dir = project_path.join(".oj/runbooks");
...
fn print_available_commands(project_path: &Path) -> Result<()> {
    print!("{}", available_commands_help(project_path));
    Ok(())
}
...
pub async fn handle(
    ...
    let Some(ref command) = args.command else {
        return print_available_commands(project_path);
    };
    ...
    if raw_args.iter().any(|a| a == "--help" || a == "-h") {
        return print_command_help(project_path, command, args.runbook.as_deref());
    }

    // Prevalidate locally for fast feedback
    let runbook = prevalidate_command(project_path, command, args.runbook.as_deref())?;
    ...
    if let RunDirective::Shell(ref cmd) = cmd_def.run {
        return execute_shell_inline(cmd, cmd_def, &positional, &named, project_path, invoke_dir);
    }
    ...
    let result =
        client.run_command(project_path, invoke_dir, project, command, &positional, &named).await?;

    match result {
        crate::client::RunCommandResult::Job { job_id, job_name } => {
            dispatch_job(&client, project, command, &job_id, &job_name).await
        }
        crate::client::RunCommandResult::Crew { crew_id, agent_name } => {
            dispatch_crew(&client, project, command, &crew_id, &agent_name, should_attach).await
        }
    }
}
...
fn execute_shell_inline(
    ...
    let mut vars: HashMap<String, String> =
        parsed_args.iter().map(|(k, v)| (format!("args.{}", k), v.clone())).collect();
    vars.insert("invoke.dir".to_string(), invoke_dir.display().to_string());
    vars.insert("workspace".to_string(), project_path.display().to_string());

    let interpolated = oj_runbook::interpolate_shell(cmd, &vars);

    // OJ_PROJECT is NOT injected here — the CLI shell-inline path runs
    // outside engine-managed workspaces.  If OJ_PROJECT is already set
...
async fn dispatch_job(
    ...
    let short_id = oj_core::short(job_id, 8);
    println!("{} {project}", color::context("Project:"));
    ...
    if started {
        ...
        println!("{}", color::yellow("Still waiting for the job to start, check:"));
    }
...
async fn dispatch_crew(
    ...
    let short_id = oj_core::short(crew_id, 8);
    println!("{} {project}", color::context("Project:"));
    println!("{} {command}", color::context("Command:"));
    println!(
        "{} {agent_name} {}",
    ...
    loop {
        ...
        if let Ok(Some(detail)) = client.get_agent(crew_id).await {
    ...
    if agent_ready {
        crate::daemon_process::coop_attach(crew_id)?;
    } else {
        println!("{}", color::yellow("Agent session not ready yet. You can attach manually:"));
