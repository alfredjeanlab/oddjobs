...
use oj_adapters::{AgentAdapter, AgentReconnectConfig, NotifyAdapter};
use oj_core::{
    ...
    C: Clock,
{
    ...
    pub(crate) async fn spawn_standalone_agent(
        ...
        let ctx = crate::spawn::SpawnCtx::from_crew(crew_id, agent_name, project);
    ...
    pub async fn recover_standalone_agent(&self, crew: &oj_core::Crew) -> Result<(), RuntimeError> {
        ...
        let process_name = self
            .cached_runbook(&crew.runbook_hash)
            .ok()
            .and_then(|rb| {
                rb.get_agent(&crew.agent_name)
