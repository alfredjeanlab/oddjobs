...
fn prime_render_per_source() {
    ...
    map.insert("resume".to_string(), PrimeDef::Script("echo resuming ${name}".to_string()));
...
fn prime_per_source_rejects_nested() {
    ...
    assert!(result.is_err(), "nested PerSource should fail deserialization");
    let err = result.unwrap_err().to_string();
    assert!(err.contains("nested per-source prime"), "error should mention nesting: {}", err);
}
...
fn prime_per_source_render_panics() {
    let mut map = HashMap::new();
    map.insert("startup".to_string(), PrimeDef::Script("echo hi".to_string()));
    let prime = PrimeDef::PerSource(map);
    let vars = HashMap::new();
    prime.render(&vars);
}
...
fn parses_agent_with_notify() {
    ...
    "#;
    let agent: AgentDef = toml::from_str(toml).unwrap();
    assert_eq!(agent.notify.on_start.as_deref(), Some("Agent started: ${name}"));
...
fn on_idle_auto_action() {
    ...
    "#;
    let agent: AgentDef = toml::from_str(toml).unwrap();
    assert_eq!(agent.on_idle.as_ref().unwrap().action(), &AgentAction::Auto);
}

#[test]
fn auto_action_as_str() {
    assert_eq!(AgentAction::Auto.as_str(), "auto");
}

#[test]
fn auto_action_valid_for_on_idle() {
    assert!(AgentAction::Auto.is_valid_for_trigger(ActionTrigger::OnIdle));
}

#[test]
fn auto_action_invalid_for_on_dead() {
    assert!(!AgentAction::Auto.is_valid_for_trigger(ActionTrigger::OnDead));
}

#[test]
fn auto_action_invalid_for_on_prompt() {
    assert!(!AgentAction::Auto.is_valid_for_trigger(ActionTrigger::OnPrompt));
}
...
fn agent_build_command_with_prompt_field() {
    ...
    let agent = AgentDef {
        ...
        cwd: None,
        prime: None,
        on_idle: Some(ActionConfig::default()),
    ...
    // build_command just interpolates the run template; prompt is added by spawn.rs
...
fn agent_build_env() {
    ...
    let vars: HashMap<String, String> =
        [("job_id".to_string(), "job-1".to_string()), ("name".to_string(), "feature".to_string())]
            .into_iter()
            .collect();
...
fn agent_get_prompt_from_field() {
    ...
    let vars: HashMap<String, String> =
        [("task".to_string(), "coding".to_string()), ("name".to_string(), "feature-1".to_string())]
            .into_iter()
            .collect();
...
fn parses_full_agent_with_actions() {
    ...
    "#;
    let agent: AgentDef = toml::from_str(toml).unwrap();
    assert_eq!(agent.on_idle.as_ref().unwrap().action(), &AgentAction::Nudge);
    assert_eq!(agent.on_idle.as_ref().unwrap().message(), Some("Keep going"));
