...
//! Tests for pipelines, pipefail, and multi-stage pipelines.
...
#[tokio::test]
async fn pipeline_exit_code_from_last() {
    // `false | true` -> pipeline exit = 0 (last command)
    let result = executor().execute_str("false | true").await.unwrap();
    assert_eq!(result.exit_code, 0);

    // `true | false` -> pipeline exit = 1 (last command)
...
#[tokio::test]
async fn pipeline_pipefail_returns_rightmost_failure() {
...
#[tokio::test]
async fn pipeline_pipefail_all_success_returns_zero() {
...
#[tokio::test]
async fn pipeline_pipefail_multiple_failures_returns_rightmost() {
...
fn pipeline_stages(script: &str, trace_count: usize, expected: &str) {
    run_async(async {
...
#[tokio::test]
async fn pipeline_multi_stage_exit_code_from_last() {
    // Multi-stage pipeline exit code comes from last command
...
fn pipeline_multi_stage_pipefail(script: &str, expected_exit: i32) {
    run_async(async {
        let executor = ShellExecutor::new().pipefail(true);
...
#[tokio::test]
async fn pipeline_multi_stage_pipefail_all_success() {
