...
async fn job_on_start_emits_notification() {
    ...
    let args = vars!("name" => "my-feature");

    handle_event_chain(
        &ctx,
        command_event("job-1", "notified", "notified", args, &ctx.project_path),
    )
    .await;
...
async fn job_on_done_emits_notification() {
    ...
    ctx.runtime.handle_event(shell_ok("job-1", "init")).await.unwrap();
...
async fn job_on_fail_emits_notification() {
    ...
    ctx.runtime.handle_event(shell_fail("job-1", "init")).await.unwrap();
